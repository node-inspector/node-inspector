  ...
  constructor(sources) { super();
    ...
    : [INSPECTOR_PLUGINS, EXTERNAL_PLUGINS]
        ...
        .map(dir => fs.readdirSync(dir)
          .map(name => path.resolve(dir, name, 'manifest.js'))
          .filter(fs.existsSync)
    ...
    this.config = {
      commands: this.reduce((result, manifest) => {
        Object.assign(result, manifest.config.commands);
        return result;
      }, {})
    };
  }
...
class ManifestPart {
  constructor(manifest, name) {
    ...
    ['frontend', 'inspector', 'protocol', 'config'].forEach((key) => {
    ...
    this.config = new ManifestConfig(this.config, this.cwd);
  }
}
...
class ManifestConfig extends Object {
  constructor(data, cwd) {
    super(data);

    this.commands = [].concat(this.commands || [])
      .reduce((result, command) => {
        result[command.name] = new ManifestConfigCommand(command, cwd);
        return result;
      }, {});
  }
}

class ManifestConfigCommand extends Object {
  constructor(data, cwd) {
    super(data);

    this.path = path.resolve(cwd, this.path);
  }
}
