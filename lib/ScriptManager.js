...
function ScriptManager(config, session) {
  ...
  this._debuggerClient.on('afterCompile', (event) => this.add(event.script));
}
inherits(ScriptManager, EventEmitter);

ScriptManager.prototype.cwd = function() {
  return co(function * () {
...
ScriptManager.prototype.resolve = function(id) {
  return co(function * () {
...
ScriptManager.prototype.source = function(id) {
  return co(function * () {
...
ScriptManager.prototype._requireScriptFromApp = function(id) {
  return co(function * () {
    ...
    return yield this.add(scripts[0]);
  }.bind(this));
};

ScriptManager.prototype.find = function(path) {
 ...
 */
ScriptManager.prototype.get = function(id) {
  return this._sources[id];
};

ScriptManager.prototype.add = function(v8data) {
  return co(function * () {
    ...
    debug('add id: %s localPath: %s hidden? %s source? %s',
...
ScriptManager.prototype._getSourceMapUrl = function(scriptId, scriptSource) {
  return co(function * () {
    if (scriptSource == null)
      scriptSource = yield this.source(scriptId);
...
ScriptManager.prototype.list = function() {
  return co(function * () {
    var startDirectory = yield this.cwd();
    var mainAppScript = yield this.realMainAppScript();
    return yield this._scriptStorage.findAllApplicationScripts(startDirectory, mainAppScript);
  }.bind(this));
};

ScriptManager.prototype.load = function(path) {
  return co(function * () {
    var normalizedName = yield this.normalizeName(path);
    var scriptName = convert.inspectorUrlToV8Name(normalizedName);
    var scriptId = this.find(scriptName);
    ...
    if (scriptId != null)
      return yield this.source(scriptId);

    return yield this._scriptStorage.load(scriptName);
  }.bind(this));
};

ScriptManager.prototype.save = function(name, source) {
  return co(function * () {
    return yield this._scriptStorage.save(name, source);
  }.bind(this));
};
