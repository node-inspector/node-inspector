...
InjectorClient.prototype.inject = function(cb) {
  ...
  if (this.needsInject) {
    _water.unshift(
      ...
      this._restartIfPaused.bind(this),
      this._injectSheduled.bind(this),
      this._notifySheduled.bind(this),
...
InjectorClient.prototype._restartIfPaused = function(cb) {
  if (this._debuggerClient.isRunning) return cb();

  this._debuggerClient.request('restartframe', { frame: 0 }, function(error, result) {
    if (error) return cb(error);

    result = result.result || result;
    if (result.stack_update_needs_step_in)
      this._debuggerClient.request('continue', { stepaction: 'in' }, () => cb());
    else
      cb(error);
  }.bind(this));
};
...
InjectorClient.prototype._injectSheduled = function(cb) {
  var index = 0;
  var errors = [];
  var results = [];
  var iterate = function() {
    var injection = this._seduledInjections[index++];
    if (!injection) return cb(null, errors, results);

    this.injection(injection[0], injection[1], function(error, result) {
      errors.push(error);
      results.push(result);
      iterate();
    }.bind(this));
...
InjectorClient.prototype._notifySheduled = function(errors, results, cb) {
  errors.forEach(function(_, index) {
    var cb = this._seduledInjections[index][2];
    cb(errors[index], results[index]);
  }, this);
...
InjectorClient.prototype.injection = function(injection, options, callback) {
  if (!this._injected) {
    this._seduledInjections.push([injection, options, callback]);
    return;
  }
