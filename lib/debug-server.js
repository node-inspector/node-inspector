...
DebugServer.prototype.getInspectorAddress = function() {
  ...
  address.url = buildInspectorUrl(config.webHost, address.port, config.debugPort, this._isHTTPS);
  return address;
};
...
DebugServer.prototype.getWebSocketAddress = function() {
  var config = this._config;
  var address = this._httpServer.address();
  address.url = buildWebSocketUrl(config.webHost, address.port, config.debugPort, this._isHTTPS);
...
DebugServer.prototype.getFaviconUrl = function(debugPort) {
  ...
  return buildFrontendUrl(config.webHost, address.port, '/favicon.ico', null, this._isHTTPS);
