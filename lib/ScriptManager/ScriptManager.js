...
class ScriptManager extends require('events') {
  ...
  constructor(config, session) { super();
    ...
    this.fs = new FileStorage(config, session);
    ...
    session.debugger.on('afterCompile', (event) => this.add(event.script).catch(
      error => this.session.frontend.sendLogToConsole('error', error.stack)));
  }
  ...
  source(id) {
    return co(function * () {
      const result = yield this.session.request('scripts', {
        includeSource: true,
        types: 4,
        ids: [Number(id)]
      });
  ...
  list() {
    return co(function * () {
      ...
      return yield this.fs.list(startDirectory, mainAppScript);
    }.bind(this));
  }
  ...
  reload() {
    return co(function * () {
      this.reset();
      const scripts = yield this.session.request('scripts', {includeSource: true, types: 4});
  ...
  _notifyScriptParsed(inspectorScript) {
    this.session.frontend.emitEvent('Debugger.scriptParsed', inspectorScript);
  }
}
