...
function BreakEventHandler(config, session) {
  ...
  if (!this._session.resourceTreeResolved)
    this._waitForResourceTree();
}

BreakEventHandler.prototype._waitForResourceTree = function() {
  this._eventsCache = [];
  this._session.once('resource-tree-resolved', this._flushCachedEvents.bind(this));
};

BreakEventHandler.prototype._flushCachedEvents = function() {
  var events = this._eventsCache;
  process.nextTick(function() {
    events.forEach(this._onBreak, this);
  }.bind(this));

  this._eventsCache = null;
};

BreakEventHandler.prototype._cacheEvent = function(obj) {
  this._eventsCache.push(obj);
};
