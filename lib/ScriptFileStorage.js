...
function list(root, include, exclude) {
  ...
  function _list(node) {
    return co(function * () {
      try {
        const paths = (yield readdir(node))
          .map(child => path.join(node, child))
          .filter(path => exclude.every(rx => !rx.test(relative(path))));

        return Promise.all(
          paths.map(path => co(function * () {
            try {
              const path = paths[i];
              const stat = yield stat(path);
              const rpath = relative(path);

              if (cache[path]) return;
              cache[path] = true;

              if (stat.isDirectory()) return folders.push(path);
              if (stat.isFile() && include.some(rx => rx.test(rpath))) list.push(path);
            } catch (e) {}
          }))
        );
      } catch (e) {}
