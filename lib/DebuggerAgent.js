...
DebuggerAgent.prototype = {
  ...
  setScriptSource: function(params, done) {
    this._debuggerClient.request(
      ...
      function(err, response) {
        this._handleChangeLiveOrRestartFrameResponse(done, true, err, response);
        this._persistScriptChanges(params.scriptId, params.scriptSource);
      }.bind(this)
    );
  },

  _handleChangeLiveOrRestartFrameResponse: function(done,
                                                    handleStepIn,
                                                    err,
                                                    response) {
    if (err) {
    ...
    if (response.result.stack_update_needs_step_in && handleStepIn)
  ...
  restartFrame: function(params, done) {
    this._debuggerClient.request(
      ...
      this._handleChangeLiveOrRestartFrameResponse.bind(this, done, false)
    );
  },
  ...
  setVariableValue: function(params, done) {
    this._debuggerClient.evaluateGlobal('process.version', function(err, version) {
      if (version < 'v0.11.2') {
          ...
          ' Please upgrade to version v0.11.2 (unstable) or newer.');
      } else {
