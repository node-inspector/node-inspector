...
ConsoleAgent.prototype.injection = function(require, debug, options) {
  ...
  function getCallerFuncLocation() {
    ...
    try { throw new Error(); }
      ...
      //Magic 4 is:                                                                       4
      //[Error string]->[getCallerFuncLocation]->[ConsoleMessage]->[Wrapped function]->[Caller]
      var callerStackLine = e.stack.split('\n')[4];
      var _location = /\((.*):(\d+):(\d+)\)$/.exec(callerStackLine);
      location.url = _location[1];
      location.line = _location[2];
      location.column = _location[3];
    }
    return location;
  }
  ...
  function wrapFunction(fn, func) {
    return function() {
      ...
      message.parameters = message.parameters.map(function(arg) {
        return serializeAndCacheMirror(cache, makeMirror(arg));
      });
  ...
  function serializeAndCacheMirror(cache, mirror, response) {
    ...
    if (response) {
      response.refs = serializer.mirrors_.map(function(refMirror) {
        return serializeAndCacheMirror(cache, refMirror);
      });
    }
  ...
  function resolveCachedMirror(message, mirror) {
    var mirrorKey = Object.keys(message).filter(function(key) {
      return message[key].value() === mirror.value();
    })[0];

    if (mirrorKey === undefined) {
      mirrorKey = mirror.handle_ = nextHandleId++;
      message[mirrorKey] = mirror;
    }

    return message[mirrorKey];
  }

  function checkRefProperty(refs, prop) {
    if (typeof prop != 'object') return;

    //unfortunately we can't use `instanceof Array` here
    if (prop.length)
