...
exports.attachDebugger = function(port) {
  ...
  debugr = Object.create(EventEmitter.prototype, {
    ...
    // TODO(bajtos) replace all usages of request with sendDebugRequest
    request: {
    ...
    sendDebugRequest: {
      value: function(command, args, callback) {
        this.request(command, { arguments: args }, function(response) {
          if (!response.success)
            callback(response.message);
          else
            callback(null, response.body, response.refs);
        });
      }
    },
