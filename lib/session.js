...
exports.create = function (conn, debugr, debuggerPort, config) {
  ...
  return Object.create(events.EventEmitter.prototype, {
    ...
    stopProfiling: {
      value: function() {
        evaluate('process.profiler.stopProfiling("org.webkit.profiles.user-initiated.' + cpuProfileCount + '")', null, function(msg) {
          sendEvent('setRecordingProfile', { isProfiling: false });
          if (msg.success) {
            var refs = {};
            profile = {};
        ...
        evaluate('process.profiler.profileCount()', null, function(msg1) {
          ...
          if (msg1.success) {
            for (i = 0, count = msg1.body.value; i < count; i++) {
              evaluate('process.profiler.getProfile(' + i + ')', null, function(msg){
                if (msg.success) {
                  var refs = {};
                  profile = {};
                  if (msg.refs && Array.isArray(msg.refs)) {
