...
exports.create = function(debuggerPort, config) {
  ...
  function parsedScripts(msg) {
    ...
    var scripts = msg.body
    // filter out application scripts with full path 
    .filter(function(s) {
      return String(s.name).indexOf('/') > -1
    })
    .map(function(s) {
      return {
    ...
    // get ridIndex for shorten path
    var ridIndex = -1;
    var sampleScript = scripts[0];
    for (var i = 0; i < sampleScript.path.length - 1; i ++) {
      ridIndex = i;
      scripts.forEach(function(s) {
        if (s.path[i] !== sampleScript.path[i]) {
          ridIndex --;
          return;
        }
      });
    ...
    scripts.forEach(function(s) {
      ...
      if (s.path.length > 1) s.url = s.path.slice(ridIndex + 1).join('/');
      item.url = s.url;
  ...
  return Object.create(events.EventEmitter.prototype, {
    ...
    removeBreakpoint: {
      value: function(sourceID, lineNumber) {
        ...
        var bp = breakpoints[id];
        if (bp) {
          debug.request(
              'clearbreakpoint',
              { arguments: { breakpoint: bp.number }},
              function(msg) {
                if (msg.success) {
