...
exports.create = function(debuggerPort, config) {
  ...
  var currentCallFrames;

  function getScopes(first, rest) {
    console.log(first);
    if (!first) {
      sendEvent(
        'Debugger.paused', {
        details: {
          callFrames: callFrames(currentCallFrames)
        }
      })
    } else {
      currentCallFrames.push(first);
      var next = rest.shift();
      debug.request(
        'scopes',
        { arguments: { frameNumber: first.index }},
        function (msg) {
          console.log(msg);
          first.scopes = msg.body.scopes;
          getScopes(next, rest);
        }
      );
    }
  ...
  function sendBacktrace() {
    debug.request(
        ...
        function(msg) {
          currentCallFrames = [];
          var frames = msg.body.frames;
          getScopes(frames.shift(), frames);
          // sendEvent(
          //     'Debugger.paused',
  ...
  return Object.create(events.EventEmitter.prototype, {
    ...
    getProperties: {
      value: function(objectId, ownProperties) {
