 ...
 */
FrontendClient.prototype.sendResponse = function(requestId, error, result) {
  if (requestId == null) {
    throw new Error('Cannot send response without requestId');
  }

  var message = { id: requestId };
  if (error != null) {
    this._onErrorResponse(error);
...
FrontendClient.prototype._onErrorResponse = function(error) {
