...
class BreakEventHandler {
  ...
  handle(event) {
    return co(function * () {
      ...
      error => this.session.frontendClient.sendLogToConsole('error', error));
  }
  ...
  _handleSkipAllPauses() {
    return co(function * () {
      if (this._skipAllPauses)
        yield this.session.debuggerClient.request('continue');
  ...
  _skipHidden(event) {
    return co(function * () {
      ...
      if (!this.session.scriptManager.get(event.script.id))
        yield this.session.debuggerClient.request('continue', { stepaction: 'out' });
  ...
  _sendBacktraceToFrontend(event) {
    return co(function * () {
      ...
      const backtrace = yield this.session.debuggerClient.request('Debugger.getBacktrace', {
        stackTraceLimit: this.config.stackTraceLimit
      });

      this.session.frontendClient.emitEvent('Debugger.paused', {
