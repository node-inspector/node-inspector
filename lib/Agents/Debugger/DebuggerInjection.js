...
module.exports = function injection(require, debug, options) {
  ...
  debug.registerAgentCommand('Debugger.setScriptSource',
    ['scriptId', 'scriptSource', 'preview'],
    function(args, response, InjectedScript, DebuggerScript) {
      try {
        debug.setLiveEditEnabled(true);
        var result = DebuggerScript.liveEditScriptSource.apply(DebuggerScript, args);
        debug.setLiveEditEnabled(false);
        ({ 0: function() {
          // Call stack may have changed after if the edited function was on the stack.
          if (!args[2] && !this.running_) {
      ...
      } catch (e) { response.failed({ message: e || 'Unknown error.' }); }
    });
  ...
  // FIXME: debugger can steals in step actions
  // Get `console` magically wakes it up
  function __wakeup__() { clearInterval(); console; }
...
/*
  ...
  debug.registerAgentCommand('Debugger.setBreakpointByUrl',
    ...
    function _setBreakpointByUrl(args, response, InjectedScript, DebuggerScript) {
      ...
      if (!isRegex) url = debug.convert.urlToPath(url);
