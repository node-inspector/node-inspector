...
DebuggerAgent.prototype._reloadScripts = function() {
  return co(function * () {
    ...
    yield scripts.map(script => this._scriptManager.addScript(script));
  }.bind(this));
};
...
DebuggerAgent.prototype.getScriptSource = function(params) {
  return co(function * () {
    var source = yield this._scriptManager.getScriptSourceById(+params.scriptId);
...
DebuggerAgent.prototype.setScriptSource = function(params) {
  return co(function * () {
    ...
    var stack = [];
    try {
      stack = yield this.handle('getBacktrace');
    } catch (e) {}

    return yield {
      callFrames: stack,
      stackChanged: result.result.stack_modified
    };
  }.bind(this));
};
...
DebuggerAgent.prototype._persistScriptChanges = function(scriptId, newSource) {
  ...
  return this._scriptManager.save(name, newSource)
