...
function DebuggerAgent(config, session) {
  ...
  this._breakEventHandler = new BreakEventHandler(config, session);
  ...
  this.registerCommand('setScriptSource');
  ...
  this._ready = this._injection().then(() => {
    this._debuggerClient.on('break', (event) => this._breakEventHandler.handle(event));
    this._debuggerClient.on('exception', (event) => this._breakEventHandler.handle(event));
  });
}
...
DebuggerAgent.prototype._persistScriptChanges = function(scriptId, source) {
  var ERROR = 'Cannot save changes to disk:';

  if (!this._saveLiveEdit) return this._warnLiveEditDisabled();

  var script = this._scriptManager.get(scriptId);
  if (!script)
  ...
  return this._scriptManager.save(name, source)
...
DebuggerAgent.prototype.continueToLocation = function(params) {
  return co(function * () {
    ...
    yield this._breakEventHandler.setContinueToLocationBreakpointId(result.breakpointId);
    this.handle('resume');
  }.bind(this));
};
