...
class PageAgent extends require('../BaseAgent') {
  ...
  getResourceTree(params) {
    return co(function * () {
      ...
      const mainAppScript = yield this.session.scriptManager.realMainAppScript();
      const scripts = yield this.session.scriptManager.list();
  ...
  getResourceContent(params) {
    return co(function * () {
      ...
      return {
        content: params.url
        ? yield this.session.scriptManager.load(params.url)
