...
class BaseAgent extends require('events') {
  ...
  request(command, params) {
    return this.session.debuggerClient.request(scope(this, command), params);
  }
  ...
  registerEvent(event, handler) {
    ...
    this.session.debuggerClient.on(scope(this, event), handler);
  }
  ...
  emitEvent(event, message) {
    this.session.frontendClient.emitEvent(scope(this, event), message);
  }
  ...
  notify(level) {
    ...
    this.session.frontendClient.sendLogToConsole(level, args);
  }
